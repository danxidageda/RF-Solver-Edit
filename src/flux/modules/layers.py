...
class SingleStreamBlock(nn.Module):
    ...
    def forward(self, x: Tensor, vec: Tensor, pe: Tensor, info) -> Tensor:
        ...
        # #q 注入
        # if info['inject'] and info['id'] > 19:
        #     feature_name = str(info['t']) + '_' + str(info['second_order']) + '_' + str(info['id']) + '_' + info['type'] + '_' + 'Q'
        #     if info['inverse']:
        #         info['feature'][feature_name] = q.cpu()
        #     else:
        #         q = info['feature'][feature_name].cuda()
        # #k 注入
        if info['inject'] and info['id'] > 19:
            feature_name = str(info['t']) + '_' + str(info['second_order']) + '_' + str(info['id']) + '_' + info['type'] + '_' + 'K'
            if info['inverse']:
                info['feature'][feature_name] = k.cpu()
            else:
                k = info['feature'][feature_name].cuda()
